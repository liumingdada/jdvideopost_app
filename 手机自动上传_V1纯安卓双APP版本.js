...
// ***初始化变量
postNum=1 //发布数量 设置
var destDir = files.getSdcardPath() + "/Pictures/"; // 相册路径 目标路径
var videoDir = files.getSdcardPath() + "/Pictures/1600/"; // 视频库路径 .mp5 文件基地库路径 

//======界面UI

// *** main JOB 开始 *** 初始准备工作 循环中要使用 每次都是变化的
// 循环发布
for (var i = 0; i < postNum; i++) {
    // 每次循环执行的发布操作
    console.log("执行第 " + (i + 1) + " 次发布操作");
    // 移动文件 +++1. 随机取一个MP5文件 ， +++2.复制到 相册文件夹， +++3.修改为MP4，并保存变量中 返回 +++4. 更新缓存 通知系统
    var srcFileName = getRandomVideoFile(videoDir); //.mp5
    processMoveFile(srcFileName,videoDir,destDir)// 处理移动 改名 视频文件 + 更新缓存 通知系统; 先取到一个随机.MP5文件 ; 先设置好 初始化变量 ; 
...
function setAddPro(){
  ...
  press(100, 450, 1000); // 模拟点击 位置x,y,时间， 屏幕放大也正常能按到， 目前分辨率是720*1280
  sleep(500);
...
function addJDCart(skuID){
  ...
  press(100, 150, 500); //模拟按点 x,y区域100, 150 搜索框  
  press(120, 80, 500);  
  setText(skuID);
  toastLog("已输入: "+skuID);
  sleep(500); 
  ...
  if (objTmp) {      
      ...
      press(100, 300, 300);//模拟按点 商品图片 以进入商品详情页面
      sleep(2000); 
      // 点击添加到购物车按钮
...
function checkVideoPublished() {
  return new Promise((resolve, reject) => {
      var maxWaitTime = 60000; // 最大等待时间，例如60秒
...
//处理 移动 视频文件 ，改名 ， 传入 参数： 视频库中 随机一个当前视频文件名;  
